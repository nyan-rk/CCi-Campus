<?php 
    ...
    function retrieveLastThreeDiagSeen($idUser, $db)
    {
        ...
        $req=$db->prepare('SELECT v.id_diag,d.name_diag,d.desc_diag, DATE_FORMAT(max(date_viewed),"%d %m %Y") as last_modif FROM view_history as v INNER JOIN diag as d on v.id_diag=d.id_diag WHERE v.id_user=:USER GROUP BY v.id_diag ORDER BY max(date_viewed) DESC LIMIT 3');
