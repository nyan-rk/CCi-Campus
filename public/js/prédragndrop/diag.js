...
function onDrop(event) {
  ...
  if ($(dropzone).hasClass("stack")){
      dropzone.appendChild(draggableElement);
      event
      .dataTransfer
      .clearData();
      console.log("Stack parent =" + dropzone);
      console.log("inputeStack =" + inpute);
      //if ($(dropzone).find("newTask")!=undefined) alert("Je vois ton truc");
      /*inpute.parent().append(inpute);*/
      }
  if ($(dropzone).hasClass("task")){
  ...
  // Task and stack closure
    ...
    $(".row").on("keypress",".newTask",function(e){
      if (e.which == '13' && this.value.length>0) {
        ...
        newTask.id="task-"+rowmax;
        rowmax++;
        $(newTask).addClass("list-group-item task");
        newTask.draggable="true";
        newTask.addEventListener('dragstart',function onDragStart(event) {
          event
            .dataTransfer
            .setData('text/plain', event.target.id);
          event
        ...
        newCross.classList.add("closeTask");
        newCross.appendChild(document.createTextNode('x'));
        ...
        $(newInput).attr('placeholder','Nouvelle tâche...');
        $.ajax({
          url: './controller/updateDiag.php',
          type: 'post',
    ...
    $(".row").on("click","#newStackButton",function(){
      $(this).replaceWith("<input type='text' class='newStack' placeholder='Nouvelle colonne...'></input>");
      //screenshot();
  });
