<?php
    ...
    function displayDiag($idDiag,$placeholderTask,$bdd)
    {
        ...
        while($donnees=$colCursor->fetch()){
            ...
            echo "<div class='colo color".(($donnees['order_stack']-1)%4 + 1)."'><svg class='noteholes' width='288px' height='33px'><use href='./public/images/diag/note.svg#holes'></use></svg><div class='closeTask'>x</div><div class='card'><div class='card-header stack-header'>".$donnees['name_stack']."</div>";
            ...
            while($donnees2 = $rowCursor->fetch()){
                echo "<li id='task-".$donnees2['id_task']."' class='list-group-item task'><div class='closeTask'>x</div>".$donnees2['name_task']."</li>";
            }
